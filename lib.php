 ...
 *
 * @param string $filearea The file area (e.g., 'learningpathimage').
 ...
 * @return void
 */
function local_learningpath_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    global $DB, $USER;

    // Ensure correct context level
    if ($context->contextlevel != CONTEXT_SYSTEM) {
        return false;
    }

    // Validate file area
    if ($filearea !== 'learningpathimage') {
        return false;
    }
    ...
    // Extract itemid (Learning Path ID), filepath, and filename
    $itemid = array_shift($args); // First argument is the item ID
    $filename = array_pop($args); // Last argument is the file name
    ...
    // Retrieve the file
    $file = $fs->get_file($context->id, 'local_learningpath', 'learningpathimage', $itemid, $filepath, $filename);
    ...
    // Ensure the user has permission to view the file
    $record = $DB->get_record('local_learningpath', ['id' => $itemid]);
    if (!$record) {
        return false;
    }
...
/**
 * Get the learningpath image URL for a given learningpathid.
 *
 * @param int $learningpathid The learningpathid ID.
 * @return string The full URL to the learningpathid image.
 */
/**
 * Get the learning path image URL for a given learning path ID.
 *
 * @param int $learningpathid The learning path ID.
 * @return string The full URL to the learning path image.
 */
function get_learningpath_image_url($learningpathid) {
    global $CFG;
    ...
    $files = $fs->get_area_files($context->id, 'local_learningpath', 'learningpathimage', $learningpathid, 'itemid', false);
    ...
    if (!empty($files)) {
        ...
        return 'https://picsum.photos/seed/picsum/200/300'; // Default placeholder image
    }
}
...
function get_lpt_id($l_id, $t_id) {
    global $DB;
    $lpt_id = $DB->get_record_sql('select id  from {learning_plan_training}  where lpt_id=? AND t_id=?', array($l_id, $t_id));
    $lpt_id = $lpt_id->id;
    return $lpt_id;
}
...
function islp_assign_user($l_id) {
    global $DB;
    $return;
    $isexist = $DB->record_exists('learning_user_learningplan', array('lpt_id' => $l_id));
    if ($isexist > 0) {
        $return = true;
    } else {
        $return = false;
 ...
 *
 * @param int $lpt_id learning plan id
 * @return array user list.
 */
function get_learningplan_user($lpt_id) {
    global $DB;
    $users = $DB->get_records('learning_user_learningplan', array('lpt_id' => $lpt_id));
    return $users;
}
 ...
 */
function learningplan_training($lpt_id) {
    global $DB;
    $training = $DB->get_records('learning_plan_training', array('lpt_id' => $lpt_id));
    return $training;
}
...
function user_learningplan($u_id) {
    global $DB;
    $training = $DB->get_recordset_sql('SELECT lpt_id as id, (select  learning_plan  from {learning_learningplan} where id =lpt_id) as learningplan
...
function status_id($l_id, $u_id, $t_id) {
    ...
    $status = $DB->get_record_sql('select lut.id as id from {learning_plan_training} lpt inner join {learning_user_trainingplan} lut
                                on lut.lpt_id=lpt.id  where lpt.lpt_id=? AND lut.u_id=? AND lpt.t_id= ?', array($l_id, $u_id, $t_id));
    $status = $status->id;
    return $status;
}
 ...
 */
function delete_learningplan_record($table, $id, $url, $lpt_id = '') {
    global $DB;
    // Delete Department.
    if ($table == 'learning_learningplan') {
        $DB->delete_records('learning_learningplan', array('id' => $id));
        $DB->delete_records('learning_plan_training ', array('lpt_id' => $id));
        $DB->delete_records('learning_user_learningplan ', array('lpt_id' => $id));
        ...
        $DB->delete_records('learning_user_learningplan', array('u_id' => $id, 'lpt_id' => $lpt_id));
        // Remove all training in learning_user_trainingplan
        $sql = "select id from  {learning_plan_training} where lpt_id =?";
        $lpts = $DB->get_recordset_sql($sql, array('lpt_id' => $lpt_id), $limitfrom = 0, $limitnum = 0);
        foreach ($lpts as $lpt) {
 ...
 * @return table.
 */
function display_list($lpt_id, $u_id) {
    ...
    $table->data = array();
    $sql = 'select  t_id, (select type_id from  {learning_training} where id =t_id) as type_id, lpt_id, lut.status, lut.remarks, `u_id` as id,(select training_name from  {learning_training} where id =t_id)
            as training, (select url from {learning_training} where id =t_id) as url, (select learning_plan from   {learning_learningplan} where id =lpt_id) as learning_plan, (SELECT
            ...
            on lut.lpt_id=lpt.id  where lpt.lpt_id=? AND lut.u_id=?';
    $inc = 0;
    $rs = $DB->get_recordset_sql($sql, array($lpt_id, $u_id));

    if ($DB->record_exists_sql($sql, array($lpt_id, $u_id))) {
        foreach ($rs as $log) {
            $row = array();
            $row[] = ++$inc;
            if (strlen($log->url) > 0) {
